...
-(void) setAttributeValue:(NSString *)value forKey:(NSString *)key{
    ...
    if([key isEqualToString:@"noframe"]){
        if (value&&[[value lowercaseString] isEqualToString:@"true"])
        {
            
            if (self.attributes&&![self.attributes isKindOfClass:[NSMutableDictionary class]]&&[self.attributes isKindOfClass:[NSDictionary class]])
            {
                NSMutableDictionary* newDict = nil;
                newDict = [NSMutableDictionary dictionaryWithCapacity:0];
                [newDict addEntriesFromDictionary:self.attributes];
                self.attributes = newDict;
            }
            if ([self.attributes isKindOfClass:[NSMutableDictionary class]]) {
                NSString* oldWidth = [self.attributes valueForKey:@"width"];
                NSString* oldHeight = [self.attributes valueForKey:@"height"];
                NSString* tmpwidth = [self.attributes valueForKey:@"tmpwidth"];
                NSString* tmpheight = [self.attributes valueForKey:@"tmpheight"];
                if (!tmpwidth) {
                    [self.attributes setValue:oldWidth forKey:@"tmpwidth"];
                }
                if (!tmpheight) {
                    [self.attributes setValue:oldHeight forKey:@"tmpheight"];
                }
                [self.attributes setValue:@"0" forKey:@"width"];
                [self.attributes setValue:@"0" forKey:@"height"];
            }
        }
        else if([value isEqualToString:@"false"])
        {
            ...
            if (tmpheight||tmpwidth) {
                ...
                if ([self.attributes isKindOfClass:[NSMutableDictionary class]])
                {
                    ...
                    NSString* tmpheight = [self.attributes valueForKey:@"tmpheight"];
                    if (tmpwidth) {
                        [self.attributes setValue:tmpwidth forKey:@"width"];
                    }
                    if (tmpheight) {
                        [self.attributes setValue:tmpwidth forKey:@"height"];
                    }
                }
            }
        }
...
-(void) setSgtyleValue:(NSString *)value forKey:(NSString *)key{
    if(_style){
        [_style setStringValue:value forKey:key];
    }
}
