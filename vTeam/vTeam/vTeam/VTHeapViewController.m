 ...
 * 支持多手势滑动
 *
 * config.heap.simultaneously 在cfg.plist文件中ui节点下的controller节点下配置heap.simultaneously参数
 * YES 为支持多手势
 *
*/
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
    if(![[[self topController] valueForKeyPath:@"config.heap.disabled"] boolValue] && [[[self topController] valueForKeyPath:@"config.heap.simultaneously"] boolValue]){
        return YES;
    }else{
        return NO;
    }
...
-(NSString *) loadUrl:(NSURL *) url basePath:(NSString *) basePath animated:(BOOL) animated callBackDelegate:(id)delegate {
    ...
    if(delegate && [newViewControllers lastObject]){
        if([[newViewControllers lastObject] respondsToSelector:@selector(setDelegate:)]){
            [[newViewControllers lastObject] setDelegate:delegate];
        }
    }
...
-(BOOL) openUrl:(NSURL *) url animated:(BOOL) animated{
    ...
    if([[url scheme] isEqualToString:scheme]){
        ...
        [self loadUrl:url basePath:self.basePath animated:animated callBackDelegate:nil];
...
-(BOOL) openUrl:(NSURL *) url animated:(BOOL) animated callBackDelegate:(id)delegate{
    ...
    if([[url scheme] isEqualToString:scheme]){
        ...
        [self loadUrl:url basePath:self.basePath animated:animated callBackDelegate:delegate];
