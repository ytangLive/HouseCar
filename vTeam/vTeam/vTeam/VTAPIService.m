...
-(void) vtHttpTaskDidLoaded:(id) httpTask{
    ...
    [respTask setResultStr:[httpTask responseStr]];
    [respTask setUrl:[[(VTHttpTask *)httpTask request] URL]];
...
-(void) vtHttpTaskWillRequest:(id)httpTask{
    ...
    [mDic setValuesForKeysWithDictionary:[reqTask queryValues]];
    
    u = [NSURL URLWithString:url queryValues:mDic];
    ...
    for (NSString * key in httpHeaders) {
        if ([key isEqualToString:@"Referer"]&&[url hasPrefix:[httpHeaders valueForKey:key]]) {
