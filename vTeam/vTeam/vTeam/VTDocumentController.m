...
@property(nonatomic,retain) NSString * styleValue;
...
@property(nonatomic,retain) NSMutableArray * dynamicDatas;
...
-(id)init
{
    ...
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(domStyleChanged:)
                                                     name:@"DomStyleChangedNotification"
                                                   object:nil];
    }
    return self;
}
...
-(void) dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DomStyleChangedNotification" object:nil];
...
-(void)domStyleChanged:(NSNotification*)notify
{
    [self reloadStyleData];
...
-(void) addElement:(VTDOMElement *)element toDynamicDatas:(NSMutableArray *) dynamicDatas{
    if(element){
        ...
        for(NSString * key in [[element style] allKeys]){
            
            NSString * v = [[element style] stringValueForKey:key];
            
            if(v&&[v isKindOfClass:[NSString class]] && [v rangeOfString:@"~["].location != NSNotFound){
                
                VTDocumentControllerDynamicData * dynamic = [[VTDocumentControllerDynamicData alloc] init];
                
                [dynamic setStyleKey:key];
                [dynamic setStyleValue:[NSString stringWithString:v]];
                [dynamic setElement:element];
...
-(void) documentDynamicDataBind{
    ...
    for(VTDocumentControllerDynamicData * dynamic in _dynamicDatas){
        ...
        NSString * styleKey = [dynamic styleKey];
        NSString * styleValue = [dynamic styleValue];
        ...
        {
            NSString* realStyleValue = [styleValue htmlStringByDOMSource:self htmlEncoded:NO];
            [element setSgtyleValue:realStyleValue forKey:styleKey];
            if ([[styleKey lowercaseString] isEqualToString:@"src"]) {
...
-(void)reloadStyleData
{
    self.dynamicDatas = nil;
    self.document = nil;
