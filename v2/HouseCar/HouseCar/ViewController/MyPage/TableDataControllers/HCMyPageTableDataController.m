...
@interface HCMyPageTableDataController()

@end

@implementation HCMyPageTableDataController
{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (yOffset < 0) {
        ...
        if(alphaValue >= 1){
            alphaValue = 1;
            if(_toolBarView.titleLabel.hidden){
                _toolBarView.titleLabel.hidden = NO;
            }
        }else{
            if(!_toolBarView.titleLabel.hidden){
                _toolBarView.titleLabel.hidden = YES;
            }
        }
